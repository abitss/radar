...
function groqKey(){return process.env.GROQ_API_KEY||process.env.GROQ_KEY||process.env.GROQ_API_TOKEN||process.env.GROQ_TOKEN||''}
function providerEnabled(name){if(name==='groq')return Boolean(groqKey());if(name==='cloudflare')return Boolean(process.env.CLOUDFLARE_API_TOKEN&&process.env.CLOUDFLARE_ACCOUNT_ID);if(name==='gemini')return Boolean(process.env.GEMINI_API_KEY);if(name==='openrouter')return Boolean(process.env.OPENROUTER_API_KEY);return false}
...
async function callGroq(prompt,{model,web=false,external}){
  ...
  const data=await jsonFetch('https://api.groq.com/openai/v1/chat/completions',{provider:'groq',method:'POST',headers:{'content-type':'application/json',authorization:`Bearer ${groqKey()}`},body:JSON.stringify(body)});
