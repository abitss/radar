...
export async function failJob(job,error){const terminal=Number(job.attempts)>=Number(job.max_attempts);const exponential=Math.min(3600,Math.pow(2,Math.max(0,Number(job.attempts)-1))*30);const requested=Math.max(0,Number(error?.retryAfterSeconds)||0);const backoff=Math.min(3600,Math.max(exponential,requested));const message=error?.message||String(error||'Unknown job failure');await query(`UPDATE job_queue SET status=$2,last_error=$3,locked_at=NULL,locked_by=NULL,available_at=CASE WHEN $2='queued' THEN now()+($4::text||' seconds')::interval ELSE available_at END,updated_at=now() WHERE id=$1`,[job.id,terminal?'failed':'queued',message.slice(0,1000),String(backoff)])}
